...
Original file is located at
    https://colab.research.google.com/drive/14YybdfmSn1eEfgGuMsaO8LVDC9bGecD3
"""
...
def get_dataset(dataset, data_path):
    ...
    # Added for MHIST
        ...
        transform = transforms.Compose([transforms.RandomRotation(180), transforms.RandomHorizontalFlip(), transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.2), transforms.ToTensor(), transforms.Normalize(mean = mean, std = std)])


        # Load annotations
        annotations = pd.read_csv(r"D:/Documents/0_UofT_Courses/ECE1512 Digital Image Processing/ProjectB/Project_B_Supp/Project_B_Supp/mhist_dataset/annotations.csv")
        annotations.set_index("Image Name", inplace=True)

        # Function to create dataset and labels
        def create_dataset_and_labels(files, note, ratio, batch_size=32):
            dataset = []
            labels = []

            for img_path in files:
                img_name = img_path[7:]  # Use '\\' to split the path on Windows

                if annotations.loc[img_name][2] == note:
                    img = Image.open(img_path)
                    label = annotations.loc[img_name][0]
            ...
            return torch.stack(dataset), np.array(labels).reshape(-1)

        img_files = glob.glob("images/*")
        train_dataset, train_labels = create_dataset_and_labels(img_files, "train", 2, batch_size=batch_size)
        test_dataset, test_labels = create_dataset_and_labels(img_files, "test", 1, batch_size=batch_size)
        train_labels = np.where(train_labels == "HP", 0, 1) 
        test_labels = np.where(test_labels == "HP", 0, 1)  
        
        dst_train = TensorDataset(train_dataset, torch.from_numpy(train_labels))
        dst_test = TensorDataset(test_dataset, torch.from_numpy(test_labels))

        # Create PyTorch DataLoader
        trainloader = DataLoader(list(zip(train_dataset, train_labels)), batch_size=batch_size, shuffle=True)
        testloader = DataLoader(list(zip(test_dataset, test_labels)), batch_size=batch_size, shuffle=False)
    ...
    if dataset != "MHIST":
        trainloader = torch.utils.data.DataLoader(dst_test, batch_size=batch_size, shuffle=True, num_workers=0)
        testloader = torch.utils.data.DataLoader(dst_train, batch_size=batch_size, shuffle=False, num_workers=0)
...
def get_network(model, channel, num_classes, im_size=(32, 32)):
    ...
    # print("net_width = {}, net_depth = {}, net_act = {}, net_norm = {}, net_pooling  = {}".format(net_width, net_depth, net_act, net_norm, net_pooling))
    ...
    #Added for ConvNet7
        ...
        net = ConvNet(channel=channel, num_classes=num_classes, net_width=32, net_depth=7, net_act=net_act, net_norm=net_norm, net_pooling=net_pooling, im_size=im_size)
...
def epoch(mode, dataloader, net, optimizer, criterion, args, aug, optimization_steps = None):
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args["epoch_eval_train"], eta_min=args["lr_net"])
